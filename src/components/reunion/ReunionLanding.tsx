...
// Origen organico que vamos a enviar a Monday vía n8n.
// Se basa en document.referrer y utm_source para identificar la red.
function detectOrganicoSource(): string {
  if (typeof window === "undefined") return "Organico - Directo";
  const ref = (typeof document !== "undefined" ? document.referrer : "").toLowerCase();
  const qs = new URLSearchParams(window.location.search);
  const utm = (qs.get("utm_source") || "").toLowerCase();

  const matches = (needle: string) => ref.includes(needle) || utm.includes(needle);

  if (matches("facebook") || matches("fb.com") || matches("fb.me") || matches("l.facebook")) {
    return "Organico - Facebook";
  }
  if (matches("instagram") || matches("ig.com") || utm === "ig") {
    return "Organico - Instagram";
  }
  if (matches("google") || matches("youtube") || matches("youtu.be")) {
    return "Organico - Google";
  }
  return "Organico - Directo";
}
    ...
    <section style={{ position: "relative", overflow: "hidden" }}>
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse 90% 50% at 50% -10%, #E9D5FF 0%, #FBE8F0 35%, #FFFFFF 70%)",
          zIndex: 0,
          pointerEvents: "none",
        }}
      />
      ...
      >
        <div className="reveal" style={{ textAlign: "center", marginBottom: 26 }}>
          ...
          >
            Agenda tu reunión con el equipo de Clinera
          </h1>
          <p
            style={{
              fontFamily: "Inter",
          ...
          >
            Completa los siguientes pasos. 2 minutos máximo. Te confirmamos por email.
          </p>
        </div>
        ...
        <StepRole
          ...
          setLeadRole={(role) => {
            ...
            if (typeof window !== "undefined" && window.dataLayer) {
              window.dataLayer.push({ event: "reunion_select_role", lead_role: role });
            }
...
async function submitPartialLead({
  ...
  const eventId = "reunion_" + Date.now() + "_" + Math.random().toString(36).substring(2, 9);
...
async function submitBookingConfirmation({
  ...
  if (typeof window !== "undefined" && window.dataLayer) {
    window.dataLayer.push({
      event: "reunion_booking_confirmed",
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {MIGRATION_OPTIONS.map((opt) => (
          <FormOption
            key={opt.id}
            selected={migrationIntent === opt.id}
            title={opt.title}
            desc={opt.desc}
            badge={opt.priority ? "Atención prioritaria" : undefined}
...
// ============== FORM OPTION (radio-style, form-feel) ==============
function FormOption({
  selected,
  title,
  desc,
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="reunion-form-opt"
      style={{
        display: "flex",
        alignItems: "flex-start",
        gap: 12,
        padding: "13px 14px",
        border: "1px solid " + (selected ? "#0A0A0A" : "#E5E7EB"),
        borderRadius: 10,
        cursor: "pointer",
    ...
    >
      ...
      <span style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 3 }}>
        <span style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontWeight: 600, fontSize: 14.5, letterSpacing: "-0.005em", lineHeight: 1.3 }}>
            {title}
          </span>
          {badge && (
            <span
...
function StepCalCom({
  ...
  useEffect(() => {
    ...
    Cal.ns!.organico("ui", { hideEventTypeDetails: true, layout: "month_view" });
